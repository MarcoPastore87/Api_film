...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //const API_URL = 'https://690a24901a446bb9cc218971.mockapi.io/films/films';
    ...
    function fetchFilms(){
        ...
        fetch(API_URL)
        //verificare la res, convertirla in json
            .then(res => {

                //se la res non esiste(!not) viene generato un oggetto di Error
                if(!res) throw new Error('Errore nel recupero dei dati');
                
                //se ho la res, converti in Json
                return res.json()

            })
             //e se ho i dati, costruisco l elemento in pagina
            .then(data => {
                ...
                if (data.length === 0){
                    ...
                    filmList.innerHTML = `<p class="text-center text-muted mt-4">Nessun film presente.</p>`;
                    return;
